...
export default PostScreen = observer(({ route, navigation }) => {
  ...
  const { data, isLoading } = StackerNews.post(id); // 51788

  // React.useEffect(() => {
  //   if (data) {
  //     postStore.addHistoryPost({ pid: id, title: data?.item?.title });
  //   }
  // }, [data]);
  ...
  return (
    <Container>
      ...
      {isLoading ? (
        ...
        <ScrollView style={tw`flex-1 p-2`} scrollIndicatorInsets={{ right: 1 }}>
