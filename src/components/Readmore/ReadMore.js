...
const ReadMore = ({
  ...
  const onTextLayoutTwo = useCallback(
    ...
    [setHiddenTextLinesWithSeeLess, setMountHiddenTextTwo, setMountHiddenTextThree]
  );
  ...
  const onLayoutFour = useCallback(
    ...
    [setTruncatedLineOfImpactWidth, setMountHiddenTextFour, setMountHiddenTextFive]
  );
  ...
  const onLayoutHiddenTextSix = useCallback(
    ...
    [setReconciledLineOfImpactWidth, setMountHiddenTextSix, reconciledLineOfImpact]
  );
  ...
  const updateLineOfImpact = useCallback(
    ...
    [setHideEllipsis, setTruncatedLineOfImpact, setTruncatedLineOfImpactWidth, setCollapsedChildren, setIsMeasured]
  );

  const measureSeeMoreLine = useCallback(() => {
    if (numberOfLines < 1 || !lines.length || !collapsedLines.length || !seeMore || !seeMoreWidth) {
      return;
    }
    ...
    const seeMoreTextLength = `${ellipsis} ${seeMoreText}`.length + seeMoreOverlapCount;
    ...
    const charactersLengthTillSeeMore = charactersBeforeSeeMore?.trimEnd?.()?.length || 0;
    // text break position for collapsed text
    const textBreakPosition = charactersLengthTillSeeMore - seeMoreTextLength;

    const _truncatedText = _trimmedText?.substr(0, _trimmedText.length - seeMoreTextLength)?.trimEnd?.() || '';
    ...
    const modifiedChildrenObjects = getTextByChildren(children, TextComponent)
      ?.map((_childObject) => {
        ...
        if (_childObject.content.length > charactersToTraverse) {
          ...
          nodeFound = true;
          const childContent = insertAt(_childObject.content, '\n', charactersToTraverse);
          return {
            ...
            child:
              _childObject?.type === 'string'
                ? childContent
                : React.cloneElement(_childObject, _childObject.props, childContent),
          };
        }
  ...
  const commonHiddenComponentProps = {
    ...additionalProps,
    style: StyleSheet.flatten([Array.isArray(style) ? StyleSheet.flatten(style) : style, styles.hiddenTextAbsolute]),
  };
  ...
  const seeMoreTextHidingStyle = !isMeasured ? styles.transparentColor : styles.transparentBackground;
  const seeMoreContainerStyle = [
    hideEllipsis ? styles.seeMoreContainerEllpisisHidden : styles.seeMoreContainer,
    {
  ...
  useEffect(() => {
    if (!(truncatedLineOfImpactWidth || reconciledLineOfImpactWidth) || !seeMoreWidth || !textWidth) {
      return;
    }

    const _width = reconciledLineOfImpactWidth || truncatedLineOfImpactWidth || 0;
    ...
    setIsMeasured(true);
  }, [truncatedLineOfImpactWidth, reconciledLineOfImpactWidth, seeMoreWidth, textWidth]);
  ...
  return (
    <Pressable style={wrapperStyle} onPress={toggle}>
      {/* text component to measure see if see more is applicable and get lines */}
      {mountHiddenTextOne && (
        <TextComponent {...commonHiddenComponentProps} onTextLayout={onTextLayoutOne}>
          {children || ''}
        </TextComponent>
      )}
      {/* text component to measure lines with see less */}
      {mountHiddenTextTwo && (
        <TextComponent {...commonHiddenComponentProps} onTextLayout={onTextLayoutTwo}>
          {children || ''}
      ...
      {mountHiddenTextFour && (
        <TextComponent {...hiddenComponentPropsLineOfImpact} onLayout={onLayoutFour}>
          {truncatedLineOfImpact}
        </TextComponent>
      )}
      ...
      {mountHiddenTextSix && (
        <TextComponent {...hiddenComponentPropsLineOfImpact} onLayout={onLayoutHiddenTextSix}>
          {reconciledLineOfImpact}
        </TextComponent>
      )}
      {/* actual text component */}
      <TextComponent {...additionalProps} {...restProps} style={style} {...textProps}>
        {seeMore && collapsed ? collapsedChildren || children || '' : children || ''}
        {seeMore && !collapsed && !expandOnly && (
          <TextComponent {...additionalProps} {...restProps} onPress={toggle} style={seeLessStyle}>
      ...
      {seeMore && collapsed && afterCollapsed && (
        <View style={[seeMoreContainerStyle, seeMoreContainerStyleSecondary]} onLayout={onSeeMoreViewLayout}>
          {!hideEllipsis && (
            <TextComponent
              ...
              {...restProps}
              onPress={toggle}
              style={[style, seeMoreTextHidingStyle, hideEllipsis ? styles.transparentColor : {}]}>
              {`${ellipsis} `}
            </TextComponent>
          )}
          <TextComponent
...
ReadMore.propTypes = {
  ...
  customTextComponent: PropTypes.oneOfType([PropTypes.node, PropTypes.element, PropTypes.elementType]),
